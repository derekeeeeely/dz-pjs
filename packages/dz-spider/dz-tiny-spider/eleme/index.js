    ...
    if (error) {
      ...
      if (res.body) {
        ...
        if (list && list.items && list.items.length) {
          ...
          for (let i = 0; i < final.length; i++) {
            if (!~shopLists.indexOf(final[i].shopId)) {
              ...
              geocoder(final[i].address).then(cp => {
                ...
                sqlquery("INSERT INTO eleme1101 SET ?", final[i]).then((result) => {
          ...
          // })).then(() => {
          //   Promise.all(nowAll.map(item => {
          //     return sqlquery(`select * from eleme1101 where shopId =${item.shopId}`).then(_res => {
          //       if (!(_res && _res.length)) {
          //         sqlquery("INSERT INTO eleme1101 SET ?", item).then((result) => {
          ...
          if (block.lat <= block.YH - step) {
            ...
            if (block.lgt <= block.XH - step) {
              ...
              sqlquery(`update blocks2 set status =1 where blockId=${block.blockId}`).then(res => {
              })
              done();
            }
          }
...
const runNext = (index, block) => {
  ...
  console.log(`选取随机proxy：${aproxys[i]}, block为 ${block.blockId} ：${JSON.stringify(block)}`)
  const queue = {
    headers: {
...
// 开始
const start = (city, withRemain) => {
  sqlquery('select count(*) as num from blocks2').then(res => {
    const amount = res[0].num
    let blocks = getBlocks(size, city, amount)
    for (let i = 0; i < blocks.length; i++) {
      const item = {
      ...
      sqlquery('insert into blocks2 set ?', item).then(res => {

      })
    }

    getProxys().then(e => setTimeout(() => {
      sqlquery(`select content from blocks2 where status=0`).then((__res) => {
        const remainblocks = __res.map(item => JSON.parse(item.content))
        if (withRemain) {
          blocks = blocks.concat(remainblocks)
        }
        sqlquery('select shopId from eleme1101').then(_item => {
...
const insert = () => {
  for (let i = 0; i < allList.length; i++) {
    ...
    sqlquery(`select shopId from eleme1101`).then(_res => {
...
start([beijing,shanghai,hangzhou,shenzhen,guangzhou,wuhan,nanjing,chengdu,shenyang,suzhou,wenzhou], true)
